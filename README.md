...
### 5. Git 발행 (`auto_publish_approved.py`)
- Notion에서 `검토 완료`(또는 `AUTO_PUBLISH_STATUS`) 글을 조회
- Notion 본문 → 마크다운 변환 → Hugo `content/post/<카테고리>/<slug>/index.md` 생성
- `git add/commit/pull --rebase/push origin main` → GitHub Actions가 Hugo 빌드·배포
- 1회 발행 상한 `AUTO_PUBLISH_MAX`(기본 5)로 백로그 대량 발행 방지
